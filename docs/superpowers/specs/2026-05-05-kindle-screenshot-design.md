...
起動 → 引数パース
  ...
  → Kindle のフロントウィンドウ ID を取得
  → カウントダウン後、ループ開始:
       Kindle をアクティブ化（フォーカス奪取対策）
       右矢印キー送信（初回除く）
       --delay 秒待機
       ウィンドウ ID を指定して screencapture でキャプチャ
       Pillow で上下左右の余白を除去
       直前画像とハッシュ比較
...
主要関数:
- `get_kindle_window_id() -> int`: osascript で Kindle.app のフロントウィンドウ ID を取得
- `capture_window(window_id: int, out: Path, fmt: str, quality: int) -> None`: `screencapture -l <window_id>` でウィンドウのみキャプチャ
- `crop_margins(img: Path, top: int, bottom: int, left: int, right: int) -> None`: Pillow で上下左右の余白を除去（in-place）
